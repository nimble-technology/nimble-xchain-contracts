...
import "../../../contracts/nimble-xchain/nimbleXChain.sol";
import "../../../contracts/nimble-xchain/nimbleXChainStructs.sol";

/**
 * @title A Test Suite for the EVM nimbleXChain Contracts
 */
contract nimbleXChainTest is Test {
    ...
    /**
     * @notice Sets up the wormholeSimulator contracts and deploys nimbleXChain
    ...
    function setUp() public {
        ...
        // deploy the nimbleXChain contract
     ...
     * deserialize the nimbleXChain message correctly.
     */
    function testMessageDeserialization(bytes32 targetRecipient) public {
        ...
        bytes memory encodedMessage = nimbleXChain.encodePayload(
            nimbleXChainStructs.nimbleXChainMessage({
                payloadID: uint8(1),
        ...
        nimbleXChainStructs.nimbleXChainMessage memory results =
            nimbleXChain.decodePayload(encodedMessage);
     ...
     * with the nimbleXChain contracts.
     */
    function testRegisterEmitter(
        ...
        nimbleXChain.registerEmitter(newEmitterChainId, newEmitterAddress);

        // verify that the contract state was updated correctly
        bytes32 emitterInContractState = nimbleXChain.getRegisteredEmitter(
            ethereumChainId
        );
    ...
    function testUpdateRelayerFee(uint32 relayerFeePercentage) public {
        vm.assume(
            relayerFeePercentage < nimbleXChain.getFeePrecision() &&
            relayerFeePercentage != nimbleXChain.getRelayerFeePercentage()
        );
        ...
        assertEq(nimbleXChain.getRelayerFeePercentage(), relayerFeePercentage);
    }
    ...
    function testUpdateRelayerFeeNotOwner(uint32 relayerFeePercentage) public {
        ...
        vm.expectRevert("caller not the owner");
        nimbleXChain.updateRelayerFee(relayerFeePercentage, relayerFeePrecision);
    }
    ...
    function testSendTokensWithPayload(
        ...
        // approve the nimbleXChain contract to spend wavax
        SafeERC20.safeApprove(
            IERC20(address(wavax)),
            address(nimbleXChain),
            amount
        );

        // call the source nimbleXChain contract to transfer tokens to ethereum
        uint64 sequence = nimbleXChain.sendTokensWithPayload(
            address(wavax),
            amount,
            ethereumChainId,
        ...
        assertEq(transfer.tokenChain, nimbleXChain.getChainId());
        assertEq(transfer.to, ethereumEmitter);
        assertEq(transfer.toChain, ethereumChainId);
        assertEq(transfer.fromAddress, addressToBytes32(address(nimbleXChain)));
        assertEq(transfer.amount > 0, true);

        // verify VAA values
        assertEq(wormholeMessage.sequence, sequence);
        assertEq(wormholeMessage.nonce, 0); // batchID
        assertEq(wormholeMessage.consistencyLevel, nimbleXChain.getWormholeFinality());

        // parse additional payload and verify the values
        nimbleXChainStructs.nimbleXChainMessage memory message =
            nimbleXChain.decodePayload(transfer.payload);
    ...
    function testSendTokensWithPayloadAmountZero() public {
        ...
        nimbleXChain.sendTokensWithPayload(
            token,
            amount,
            ethereumChainId,
            0, // opt out of batching
    ...
    /**
     * @notice This test confirms that nimbleXChain correctly redeems wrapped
    ...
    function testRedeemTransferWithPayloadWrappedToken(uint256 amount) public {
        vm.assume(
            amount > 1e10 &&
            amount < type(uint256).max / nimbleXChain.getRelayerFeePercentage()
        );
        ...
        // encode the message by calling the encodePayload method
        bytes memory encodednimbleXChainMessage = nimbleXChain.encodePayload(
            nimbleXChainStructs.nimbleXChainMessage({
                payloadID: uint8(1),
        ...
        // nimbleXChain contract will emit.
        ITokenBridge.TransferWithPayload memory transfer =
            ITokenBridge.TransferWithPayload({
                ...
                to: addressToBytes32(address(nimbleXChain)),
                toChain: nimbleXChain.getChainId(),
                fromAddress: ethereumEmitter,
                payload: encodednimbleXChainMessage
            });
        ...
        // register the emitter on the source contract
        nimbleXChain.registerEmitter(ethereumChainId, ethereumEmitter);
        ...
        nimbleXChain.redeemTransferWithPayload(signedMessage);
        ...
        uint256 relayerFee = nimbleXChain.calculateRelayerFee(denormalizedAmount);
    ...
    /**
     * @notice This test confirms that nimbleXChain correctly redeems native
     * tokens to the encoded recipient and handles relayer payments correctly.
     * In this case, native tokens means that the token originates on the
     * target blockchain (the nimbleXChain chain).
     */
    function testRedeemTransferWithPayloadNativeToken(
        ...
        /**
         * Create a simulated version of the wormhole message that the
         * nimbleXChain contract will emit. The token bridge will convert
        ...
        ITokenBridge.TransferWithPayload memory transfer =
            ITokenBridge.TransferWithPayload({
                ...
                tokenChain: nimbleXChain.getChainId(),
                to: addressToBytes32(address(nimbleXChain)),
                toChain: nimbleXChain.getChainId(),
                fromAddress: ethereumEmitter,
                payload: encodednimbleXChainMessage
    ...
    function testRedeemTransferWithPayloadWrappedTokenZeroRelayerFee(
        ...
        nimbleXChain.updateRelayerFee(0, relayerFeePrecision);
        assertEq(nimbleXChain.getRelayerFeePercentage(), 0);
    ...
    /**
     * @notice This test confirms that the nimbleXChain contract reverts
    ...
    function testRedeemTransferWithPayloadUnattestedToken() public {
        ...
        /**
         * Create a simulated version of the wormhole message that the
         * nimbleXChain contract will emit. Encode an unattested token
         * in the payload.
         */
        ITokenBridge.TransferWithPayload memory transfer =
            ITokenBridge.TransferWithPayload({
     ...
     /**
     * @notice This test confirms that nimbleXChain correctly reverts
    ...
    function testRedeemTransferWithPayloadInvalidSender() public {
        ...
        // NOTE: We purposely don't register an emitter on the nimbleXChain contract
