...
import "./nimbleXChainGovernance.sol";
import "./nimbleXChainMessages.sol";

/**
 * @title A Cross-Chain nimbleXChain Application
 * @notice This contract uses Wormhole's token bridge contract to send tokens
 * cross chain with an aribtrary message payload.
 */
contract nimbleXChain is nimbleXChainGovernance, nimbleXChainMessages, ReentrancyGuard {
    using BytesLib for bytes;

    event TrackingTransaction(
        string message,
     ...
     * feePrecision and relayerFeePercentage state variables. See nimbleXChainState.sol
    ...
    /**
     * @notice Transfers specified tokens to any registered nimbleXChain contract
     ...
     * - a registered nimbleXChain contract does not exist for the `targetChain`
    ...
    function sendTokensWithPayload(
        ...
        uint256 wormholeFee = getWormhole().messageFee();
        ...
        /**
         * Encode instructions (nimbleXChainMessage) to send with the token transfer.
        ...
        ITokenBridge bridge = getTokenBridge();
        ...
        emit TrackingTransaction("Approve from nimbleSC to wormholeBridge", address(this), address(bridge), amountReceived);
     ...
     * `nimbleXChainMessage` payload with additional transfer instructions.
    ...
    function redeemTransferWithPayload(bytes memory encodedTransferMessage) public {
         ...
         */
        IWormhole.VM memory parsedMessage = getWormhole().parseVM(
        ...
        bool flag;
        uint256 value;
        (flag, value) = SafeMath.trySub(getBalance(localTokenAddress), balanceBefore);
        require(flag == true, "Sub failed");
        uint256 amountTransferred = value;
        ...
        // confirm that the message sender is a registered nimbleXChain contract
        require(
            transfer.fromAddress == getRegisteredEmitter(parsedMessage.emitterChainId),
            "emitter not registered"
        );

        // parse the nimbleXChain payload from the `TransferWithPayload` struct
        nimbleXChainMessage memory nimbleXChainPayload = decodePayload(
            transfer.payload
        );
        ...
        address recipient = bytes32ToAddress(nimbleXChainPayload.targetRecipient);
        ...
        if (relayerFee == 0 || msg.sender == recipient) {
            ...
            emit TrackingTransaction("Transfer from nimbleSC to userWallet", address(this), recipient, amountTransferred);
        } else {
            // pay the relayer
            SafeERC20.safeTransfer(
                IERC20(localTokenAddress),
            ...
            emit TrackingTransaction("Transfer from nimbleSC to relayerWallet", address(this), msg.sender, relayerFee);

            // send the tokens (less relayer fees) to the recipient
            (flag, value) = SafeMath.trySub(amountTransferred, relayerFee);
            require(flag == true, "Sub failed");
            uint256 amountForRecipient = value;
            ...
            emit TrackingTransaction("Transfer from nimbleSC to userWallet", address(this), recipient, amountForRecipient);
        }
    }
    ...
    function calculateRelayerFee(uint256 amount) public view returns (uint256) {
        bool flag;
        uint256 value;
        (flag, value) = SafeMath.tryMul(amount, getRelayerFeePercentage());
        require(flag == true, "Mul failed");
        uint256 temp = value;
        (flag, value) = SafeMath.tryDiv(temp, getFeePrecision());
        require(flag == true, "Div failed");
        uint256 relayerFee = value;
        return relayerFee;
    }
    ...
    function fetchLocalAddressFromTransferMessage(
        ...
        if (tokenChain != getChainId()) {
            // identify wormhole token bridge wrapper
            localAddress = getTokenBridge().wrappedAsset(tokenChain, sourceAddress);
    ...
    function custodyTokens(
        ...
        emit TrackingTransaction("Transfer from userWallet to nimbleSC", msg.sender, address(this), amount);

        bool flag;
        uint256 value;
        (flag, value) = SafeMath.trySub(getBalance(token), balanceBefore);
        require(flag == true, "Sub failed");
