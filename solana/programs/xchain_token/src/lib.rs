...
// declare_id!("2n1p8eEw2CDiwastC3rVPYPEJzZdUpBBhJJWMTwLYty3");
declare_id!("EcNezEFFnaWCoWqPMgBPV8b574Js3QBUiLqizvM11ZF3");

#[program]
pub mod xchain_token {
    use super::*;
    ...
    pub fn initialize(
        ...
        sender_config.bump = *ctx
            .bumps
            .get("sender_config")
            .ok_or(XChainError::BumpNotFound)?;
    ...
    pub fn register_foreign_contract(
        ...
        require!(
            ...
            XChainError::InvalidForeignContract,
        );
    ...
    pub fn send_native_tokens_with_payload(
        ...
        require!(truncated_amount > 0, XChainError::ZeroBridgeAmount);
        ...
        // Serialize XChainMessage as encoded payload for Token Bridge
        // transfer.
        let payload = XChainMessage::Payload {
    ...
    pub fn redeem_native_transfer_with_payload(
        ...
        require!(
            ...
            XChainError::AlreadyRedeemed
        );

        // The intended recipient must agree with the recipient.
        let XChainMessage::Payload { recipient } = ctx.accounts.vaa.message().data();
        require!(
        ...
        if ctx.accounts.payer.key() != ctx.accounts.recipient.key() {
            ...
            require!(
                ...
                XChainError::NonExistentRelayerAta
            );
            ...
            let amount_subtracted = amount;
            let res = amount_subtracted.checked_sub(relayer_amount);
            require!(res != None, XChainError::InvalidCalculation);
    ...
    pub fn send_wrapped_tokens_with_payload(
        ...
        require!(amount > 0, XChainError::ZeroBridgeAmount);
        require!(
            recipient_chain > 0
                && recipient_chain != wormhole::CHAIN_ID_SOLANA
                && !recipient_address.iter().all(|&x| x == 0),
