...
use super::{
    ...
    XChainError, PostedXChainMessage,
};
...
pub struct RegisterForeignContract<'info> {
    ...
    #[account(
        has_one = owner @ XChainError::OwnerOnly,
        seeds = [SenderConfig::SEED_PREFIX],
        bump
    )]
...
pub struct SendNativeTokensWithPayload<'info> {
    ...
    #[account(
        address = config.token_bridge.config @ XChainError::InvalidTokenBridgeConfig
    )]
    ...
    #[account(
        address = config.token_bridge.authority_signer @ XChainError::InvalidTokenBridgeAuthoritySigner
    )]
    /// CHECK: Token Bridge authority signer. Read-only.
    pub token_bridge_authority_signer: UncheckedAccount<'info>,

    #[account(
        address = config.token_bridge.custody_signer @ XChainError::InvalidTokenBridgeCustodySigner
    )]
    ...
    #[account(
        mut,
        address = config.token_bridge.wormhole_bridge @ XChainError::InvalidWormholeBridge,
    )]
    ...
    #[account(
        mut,
        address = config.token_bridge.emitter @ XChainError::InvalidTokenBridgeEmitter
    )]
    ...
    #[account(
        mut,
        address = config.token_bridge.sequence @ XChainError::InvalidTokenBridgeSequence
    )]
    ...
    #[account(
        mut,
        address = config.token_bridge.wormhole_fee_collector @ XChainError::InvalidWormholeFeeCollector
    )]
...
pub struct RedeemNativeTransferWithPayload<'info> {
    ...
    #[account(
        mut,
        constraint = payer.key() == recipient.key() || payer_token_account.key() == anchor_spl::associated_token::get_associated_token_address(&payer.key(), &mint.key()) @ XChainError::InvalidPayerAta
    )]
    ...
    #[account(
        ...
        bump,
        constraint = foreign_contract.verify(&vaa) @ XChainError::InvalidForeignContract
    )]
    ...
    #[account(
        ...
        bump,
        seeds::program = wormhole_program,
        constraint = vaa.data().to() == *program_id || vaa.data().to() == config.key() @ XChainError::InvalidTransferToAddress,
        constraint = vaa.data().to_chain() == wormhole::CHAIN_ID_SOLANA @ XChainError::InvalidTransferToChain,
        constraint = vaa.data().token_chain() == wormhole::CHAIN_ID_SOLANA @ XChainError::InvalidTransferTokenChain
    )]
    /// Verified Wormhole message account. The Wormhole program verified
    /// signatures and posted the account data here. Read-only.
    pub vaa: Box<Account<'info, PostedXChainMessage>>,
    ...
    #[account(
        address = foreign_contract.token_bridge_foreign_endpoint @ XChainError::InvalidTokenBridgeForeignEndpoint
    )]
    /// Token Bridge foreign endpoint. This account should really be one
...
pub struct RedeemWrappedTransferWithPayload<'info> {
    ...
    #[account(
        mut,
        constraint = payer.key() == recipient.key() || payer_token_account.key() == anchor_spl::associated_token::get_associated_token_address(&payer.key(), &token_bridge_wrapped_mint.key()) @ XChainError::InvalidPayerAta
    )]
    /// CHECK: Payer's token account. If payer != recipient, must be an
    ...
    #[account(
        address = config.token_bridge.mint_authority @ XChainError::InvalidTokenBridgeMintAuthority
    )]
    /// CHECK: Token Bridge custody signer. Read-only.
    pub token_bridge_mint_authority: UncheckedAccount<'info>,
