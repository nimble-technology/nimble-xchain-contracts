...
export class DummyPlugin implements Plugin<WorkflowPayload> {
  ...
  async handleWorkflow(
    ...
    const complete = await execute.onEVM({
      chainId: 5, // EVM chain to get a wallet for
      f: async (wallet, chainId) => {
        ...
        const polygonConfig = nnull(
          configs.commonEnv.supportedChains.find(c => c.chainId === wh.CHAIN_ID_POLYGON),
        );

        var complete = false;
        try {
          complete = await wh.getIsTransferCompletedEth(
            nnull(polygonConfig.tokenBridgeAddress),
            wallet.wallet.provider,
            vaa.bytes
          );
        } catch (error) {
          console.error("==== Failed to execute arb plan ==== ", error);
        };

        this.logger.info(
          `Inside action, have wallet complete ${complete} on chain ${chainId}`,
        );
        return complete;
      },
    });
