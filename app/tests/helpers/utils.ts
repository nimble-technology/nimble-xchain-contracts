...
import {WORMHOLE_MESSAGE_EVENT_ABI, WORMHOLE_TOPIC} from "../eth-avax/testnet/constants";
import * as fs from "fs";

export function readnimbleXChainContractAddress(chain: number): string {
  return JSON.parse(
    fs.readFileSync(
      `${__dirname}/../../build/broadcast/deploy_xchain_transfer.sol/${chain}/run-latest.json`,
      "utf-8"
    )
...
export function readWormUSDContractAddress(chain: number): string {
  return JSON.parse(
    fs.readFileSync(
      `${__dirname}/../../build/broadcast/deploy_wormUSD.sol/${chain}/run-latest.json`,
      "utf-8"
    )
  ).transactions[0].contractAddress;
}

export function readTestERC20ContractAddress(chain: number): string {
  return JSON.parse(
    fs.readFileSync(
      `${__dirname}/../../build/broadcast/deploy_testERC20.sol/${chain}/run-latest.json`,
      "utf-8"
    )
  ).transactions[0].contractAddress;
}
